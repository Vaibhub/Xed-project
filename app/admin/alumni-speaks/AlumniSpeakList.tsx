...
export default function AlumniSpeakList() {
  ...
  const deleteMutation = useDeleteAlumniSpeak();
  const statusMutation = useUpdateAlumniSpeakStatus();
  const updateOrderMutation = useUpdateAlumniSpeaksOrder();
                ...
                {rows.length > 0 ? (
                  rows.map((item) => (
                    <SortableRow key={item.id} item={item}>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-4">
                          <div className="relative h-14 w-24 rounded-lg bg-slate-100 border overflow-hidden">
                            <img
                              src={item.thumbnail_image}
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end gap-1">
                          ...
                          <Button
                            ...
                            size="icon"
                            onClick={() => {
                              if (confirm(`Delete ${item.speaker_name}?`)) {
                                deleteMutation.mutate(item.id);
                              }
                            }}
                          >
